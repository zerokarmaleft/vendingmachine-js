...
var InteractiveVendingMachine = React.createClass({
  getInitialState: function() {
    return {
      coinInserted: false
    };
  ...
  render: function() {
    ...
    return (
        <div>
          <h2>Interactive Vending Machine</h2>
          <div id='status'>
            <h4>Status: {status}</h4>
          </div>
          <div id='coin-slot'>
...
React.render(
  <InteractiveVendingMachine coins      = {csp.chan()} 
                             chocolates = {csp.chan()}
                             toffees    = {csp.chan()} />,
...
React.render(
    ...
    />,
  document.getElementById('vending-machine-sim')
);
